...
export default function AdminTemplatesPage() {
  ...
  return (
    <PageLayout
      ...
      title={t("admin.templates.title", { name: (locale === "vi" ? lesson?.title_vi || lesson?.title_en : lesson?.title_en || lesson?.title_vi) || "" })}
      actions={
    ...
    >
      <div className="py-6 space-y-4">
        {lessonQuery.isLoading && (
          <Card className="p-6 text-sm text-[var(--text-secondary)]">{t("admin.templates.loading")}</Card>
        )}
          ...
          <div className="w-full max-w-4xl rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] p-6 shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between border-b border-[var(--border-subtle)] pb-4 mb-4">
              <h2 className="text-xl font-bold text-[var(--text-primary)]">
                {editingTemplate ? t("admin.templates.modalEditTitle") : t("admin.templates.modalCreateTitle")}
              </h2>
              <button
                onClick={() => {
                  setShowModal(false);
            ...
            <div className="flex-1 overflow-y-auto pr-1">
              <QuestionTemplateForm
                title={editingTemplate ? t("admin.common.edit") : t("admin.templates.add")}
