...
export const churchUnitApi = {
  ...
  classScores(id: string) {
    return apiRequest<any>(`/church-units/${encodeURIComponent(id)}/class-scores`, {
      token: getStoredTokens()?.accessToken,
    });
  },
};
...
export function useClassScoresQuery(id: string, enabled = true) {
  return useQuery({
    enabled: enabled && Boolean(id),
    queryFn: () => churchUnitApi.classScores(id),
    queryKey: [...churchUnitKeys.all, "detail", id, "class-scores"] as const,
  });
}
