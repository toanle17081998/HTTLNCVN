...
export type PageListResult = {
  items: Array<
    Pick<Page, "deleted_at" | "id" | "route_path" | "slug" | "status" | "title_en" | "title_vi" | "updated_at">
  >;
};
...
export const pageApi = {
  ...
  restore(id: string) {
    return apiRequest<Page>(`/pages/${encodeURIComponent(id)}/restore`, {
      method: "POST",
      token: getStoredTokens()?.accessToken,
    });
  },
