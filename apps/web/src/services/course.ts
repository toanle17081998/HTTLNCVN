...
export const courseApi = {
  ...
  finishAttempt(attemptId: string, answers?: Record<string, string>) {
...
export function useFinishAttemptMutation(attemptId: string) {
  ...
  return useMutation({
    mutationFn: (answers?: Record<string, string>) => courseApi.finishAttempt(attemptId, answers),
    onSuccess(attempt) {
      queryClient.setQueryData(courseKeys.attempt(attempt.id), attempt);
    },
  });
...
const categoryApi = {
  ...
  create: (dto: CreateCourseCategoryDto) =>
    apiRequest<CourseCategory>("/course-categories", { method: "POST", body: JSON.stringify(dto) }),
  update: (id: string, dto: UpdateCourseCategoryDto) =>
    apiRequest<CourseCategory>(`/course-categories/${id}`, { method: "PATCH", body: JSON.stringify(dto) }),
  delete: (id: string) =>
    apiRequest<void>(`/course-categories/${id}`, { method: "DELETE" }),
};
...
export function useCreateCourseCategoryMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (dto: CreateCourseCategoryDto) => categoryApi.create(dto),
    onSuccess() {
...
export function useUpdateCourseCategoryMutation(id: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (dto: UpdateCourseCategoryDto) => categoryApi.update(id, dto),
    onSuccess() {
      queryClient.invalidateQueries({ queryKey: categoryKeys.all });
      queryClient.invalidateQueries({ queryKey: courseKeys.all });
    },
