...
import {
  ...
  getColumnLeftPercentFromTemplate,
  getEditableColumnTemplate,
...
export const ModularEditor = forwardRef(function ModularEditor(
  ...
  const initialEditorHtmlRef = useRef(initialValue || "<p><br></p>");
  ...
  const initialEditorMode: EditorMode =
    initialValue && !isRichTextHtml(initialValue) ? "markdown" : "rich";

  const [contentHtml, setContentHtml] = useState(initialValue);
  ...
  const [richHasContent, setRichHasContent] = useState(
    initialEditorMode === "rich" && hasMeaningfulRichHtml(initialValue),
  );
  ...
  function switchEditorMode(nextMode: EditorMode) {
    ...
    if (nextMode === "markdown") {
      const savedHtml = prepareRichHtmlForSave();
      setMarkdownContent(savedHtml);
  ...
  return (
    <div className={cn("flex min-w-0 max-w-full flex-col overflow-hidden rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]", className)}>
      ...
      <div className="relative min-w-0 flex-1 overflow-hidden">
        {editorMode === "rich" ? (
          ...
          <textarea
            ...
            className="min-h-[20rem] w-full resize-none bg-[var(--bg-surface)] px-5 py-4 font-mono text-sm leading-7 text-[var(--text-primary)] outline-none placeholder:text-[var(--text-tertiary)]"
            onChange={(e) => {
